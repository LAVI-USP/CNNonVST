...
for reduc in reducFactors:
    ...
    for idX, path in enumerate(paths):
        ...
        save_figure(reduDose_rlzs[ii_phantom:ii_phantom + w, jj_phantom:jj_phantom + w],
                    '{}_01_Mammo_R_CC'.format(mAsReducFactors))
    ...
    for idX, path in enumerate(paths):
        ...
        save_figure(restDose_MB_rlzs[ii_phantom:ii_phantom + w, jj_phantom:jj_phantom + w],
                    'MB_{}_01_Mammo_R_CC'.format(mAsReducFactors))
        break
    ...
    for indDL, DL_type in enumerate(DL_types):
        ...
        # DL restored doses
        paths = list(
            pathlib.Path(path2Read + "Restorations/31_" + str(mAsReducFactors)).glob(DL_type + '*'))
        if paths == []:
        ...
        for idZ, path in enumerate(paths):
            ...
            save_figure(restDose_DL_rlzs[ii_phantom:ii_phantom + w, jj_phantom:jj_phantom + w],
                        'DL-{}_{}_01_Mammo_R_CC'.format(DL_type, mAsReducFactors))
            break
